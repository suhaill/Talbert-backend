...
//                print_r($result);die;
    ...
    /**
     * @Route("/api/plywood/updateLineItemNumber")
     * @Method({"POST"})
     * Security("is_granted('ROLE_USER')")
     */
    public function updateLineItemNumberAction(Request $request){
        ...
        try {
            ...
            if (!empty($sno) && !empty($id) && !empty($quoteId)){
                $plywood = $this->getDoctrine()->getRepository('AppBundle:Plywood')->findOneBy([
                    'id'=>$id,'quoteId'=>$quoteId
                ]);
                if(!empty($plywood)){
                    $em = $this->getDoctrine()->getManager();
                    $plywood->setLineItemNum($sno);
                    $em->persist($plywood);
                    $em->flush();
                    $flag= true;
                    $statusCode=200;
                    ...
                    $flag=false;
                    $msg='There is no plywood for this id!';
                    $statusCode=422;
                }
            } else {
                $flag=false;
