...
//                ->getSQL();
    ...
    /**
     * @Route("/api/veneer/updateLineItemNumber")
     * @Method({"POST"})
     * Security("is_granted('ROLE_USER')")
     */
    public function updateLineItemNumberAction(Request $request){
        ...
        try {
            ...
            if (!empty($sno) && !empty($id) && !empty($quoteId)){
                $veneer = $this->getDoctrine()->getRepository('AppBundle:Veneer')->findOneBy([
                    'id'=>$id,'quoteId'=>$quoteId
                ]);
                if(!empty($veneer)){
                    $em = $this->getDoctrine()->getManager();
                    $veneer->setLineItemNum($sno);
                    $em->persist($veneer);
                    $em->flush();
                    $flag= true;
                    $statusCode=200;
                    ...
                    $flag=false;
                    $msg='There is no venner for this id!';
                    $statusCode=422;
                }
            } else {
                $flag=false;
